...
import {
  LuHouse, LuPlay, LuPause, LuSkipBack, LuSkipForward,
  LuMaximize2, LuMinimize2, LuVolume2, LuVolume,
} from 'react-icons/lu'
  ...
  for (const raw of vtt.split('\n')) {
    ...
    if (line === '' || line === 'WEBVTT') {
      if (inCue && currentText.length > 0) {
        cues.push({ startTime: currentStart, endTime: currentEnd, text: currentText.join('<br />') })
      }
      currentText = []
      inCue = false
      continue
    ...
    if (m) {
      ...
      currentStart = parseVttTimestamp(`${m[1] ? m[1] + ':' : ''}${m[2]}:${m[3]}.${m[4]}`)
      currentEnd = parseVttTimestamp(`${m[5] ? m[5] + ':' : ''}${m[6]}:${m[7]}.${m[8]}`)
      currentText = []
      inCue = true
      continue
    }
...
export function VideoPlayer() {
  ...
  const [indicator, setIndicator] = useState<{ type: string; label?: string } | null>(null)
  ...
  const {
    filePath, fileName, streamUrl, blobUrl, transcodeUrl,
    needsTranscode, timeOffset,
    playing, currentTime, duration, activeSubtitleTrack,
    setPlaying, setCurrentTime, setDuration, close,
    enableTranscoding, fetchSubtitleTracks,
  } = usePlayerStore()
  ...
  useEffect(() => {
    ...
    const onError = () => {
      ...
      if (streamUrl && !blobUrl && !transcodeUrl) {
        fallbackToBlob()
      }
    }
    ...
    const onLoadedMetadata = () => {
      console.log('[video] loadedmetadata', video.videoWidth, video.videoHeight)
      if (video.videoWidth === 0 && !needsTranscode && !triedTranscode.current) {
        console.log('[video] videoWidth=0 → codec unsupported, switching to transcoding')
        triedTranscode.current = true
        enableTranscoding()
        return
      }
  ...
  }, [src, setPlaying, setCurrentTime, setDuration, fallbackToBlob, needsTranscode, timeOffset, enableTranscoding, streamUrl, blobUrl, transcodeUrl])
  ...
  useEffect(() => {
    if (filePath) {
      ...
      usePlayerStore.setState({ subtitleTracks: [], activeSubtitleTrack: null })
    }
  ...
  const togglePlay = useCallback(() => {
    usePlayerStore.setState((s) => ({ playing: !s.playing }))
  }, [])
  ...
  useEffect(() => {
    const cues = activeSubtitleTrack === EXT_SUBTITLE_ID ? externalCues : subtitleCues
    const cue = cues.find((c) => c.startTime <= currentTime && currentTime < c.endTime)
    setActiveCue(cue ?? null)
  }, [currentTime, subtitleCues, externalCues, activeSubtitleTrack])

  return (
    <div ref={containerRef} className="relative flex h-dvh flex-col overflow-hidden bg-black">
      <KeyboardHandler
      ...
      {indicator && (
        <div className="pointer-events-none absolute right-4 top-14 z-30 flex items-center gap-2 rounded-lg bg-black/60 px-3 py-2 backdrop-blur-sm">
          {indicator.type === 'play' && <LuPlay size={22} className="text-white" />}
          {indicator.type === 'pause' && <LuPause size={22} className="text-white" />}
          {indicator.type === 'forward' && <LuSkipForward size={22} className="text-white" />}
          {indicator.type === 'backward' && <LuSkipBack size={22} className="text-white" />}
          {indicator.type === 'volume-up' && <LuVolume2 size={22} className="text-white" />}
          {indicator.type === 'volume-down' && <LuVolume size={22} className="text-white" />}
          {indicator.type === 'fullscreen' && (
            indicator.label === 'ON' ? <LuMaximize2 size={22} className="text-white" /> : <LuMinimize2 size={22} className="text-white" />
          )}
          <span className="text-sm font-bold text-white">{indicator.label}</span>
        </div>
      )}
      ...
      {activeCue && (
        ...
        >
          <span
            ...
            style={{ textShadow: '2px 2px 6px rgba(0,0,0,0.95), 0 0 4px rgba(0,0,0,0.8)' }}
      ...
      <div
        className="absolute inset-x-0 top-0 z-10 flex items-center gap-2 px-2 py-1.5"
        style={{
          paddingLeft: MACOS_TRAFFIC_LIGHT_WIDTH,
      ...
      >
        <TooltipTrigger>
          ...
          <Tooltip className="rounded bg-gray-800 px-2 py-1 text-xs text-white shadow-lg">Go back</Tooltip>
        </TooltipTrigger>
        <span className="truncate text-xs text-white/60">{fileName}</span>
        {needsTranscode && (
      ...
      >
        <div className="bg-gradient-to-t from-black/80 to-transparent px-4 pb-3 pt-10">
          <PlayerControls
