  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          ref={menuRef}
          className="absolute bottom-full right-0 mb-2 min-w-[100px] overflow-hidden rounded-lg border border-border py-1 shadow-lg backdrop-blur-xl"
          ...
          {SPEEDS.map((s) => (
            <button
              ...
              className={`w-full px-3 py-1.5 text-left text-xs transition-colors hover:bg-surface-hover ${
                speed === s ? 'text-cyan-400' : 'text-text-muted'
              }`}
            >
              {s}x
            </button>
